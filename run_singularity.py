"""Singularity launch script for Alphafold docker image."""
...
flags.DEFINE_string(
    ...
    'Path to the container image to be used. It is compulsory to provide one.')
########################################
...
def _create_mount_for_singularity(mount_name: str, path: str):
    """Create a mount point for each file and directory used by the model."""
    path = pathlib.Path(path).absolute()
    target_path = pathlib.Path(_ROOT_MOUNT_DIRECTORY, mount_name)
    ...
    # mount = types.Mount(target=str(target_path), source=str(source_path),
                                            # type='bind', read_only=True)
    mount = str(source_path) + ":" + str(target_path) + ":ro"
    return mount, str(mounted_path)
...
def main(argv):
    ...
    for i, fasta_path in enumerate(FLAGS.fasta_paths):
        mount, target_path = _create_mount_for_singularity(f'fasta_path_{i}', fasta_path)
        mounts.append(mount)
    ...
    for name, path in database_paths:
        if path:
            mount, target_path = _create_mount_for_singularity(name, path)
            mounts.append(mount)
    ...
    mounts.append(FLAGS.output_dir + ":" + output_target_path + ":rw")
    mounts.append("/tmp:/tmp:rw")
...
#         logging.info(line.strip().decode('utf-8'))
    ...
    Client.load(FLAGS.container_image)

    # result = Client.run(
    resultstr = Client.run(stream = True, stream_type = "both",
                        args    = command_args,
                        nv      = True, # This allows NVIDIA devices to be accessed by the container
                        bind    = mounts,
                        # options = ['--env="' + ','.join(['NVIDIA_VISIBLE_DEVICES=' + str(FLAGS.gpu_devices),
                        #                                  'CUDA_VISIBLE_DEVICES='     + str(FLAGS.gpu_devices),
                        #                                  'TF_FORCE_UNIFIED_MEMORY=1',
                        ...
                        #                                  'TF_NUM_INTEROP_THREADS=' + f'{FLAGS.threads}',
                        #                                  'TF_NUM_INTRAOP_THREADS=' + f'{FLAGS.threads}',]) + '"',
                        options = ['--env="NVIDIA_VISIBLE_DEVICES=' + str(FLAGS.gpu_devices) + '"',
                                     '--env="CUDA_VISIBLE_DEVICES='     + str(FLAGS.gpu_devices) + '"',
                                     '--env="TF_FORCE_UNIFIED_MEMORY=1"',
                                     '--env="XLA_PYTHON_CLIENT_MEM_FRACTION=10"',
                                     '--env="TF_CPP_MIN_LOG_LEVEL=0"',
                                     '--env="OMP_NUM_THREADS=' + f'{FLAGS.threads}"',
                                     '--env="TF_NUM_INTEROP_THREADS=' + f'{FLAGS.threads}"',
                                     '--env="TF_NUM_INTRAOP_THREADS=' + f'{FLAGS.threads}"',
                                     '--writable-tmpfs'],
                        # return_result = True,
    )
